...
/******************************************************************************
  This program displays two grey scale images an On left, is an image that has come from an 
  ...
  To compile adapt the code below wo match your filenames:  
    cc -o Image_process_coursework Image_process_coursework.c -lglut -lGL -lm 
...
void detect_edges(unsigned char *in, unsigned char *out) {
  ...
  for(i=0;i<n_pixels;i++) {
    int m, p; // the pixel of interest
    int f, g, q, w; // the pixels adjacent used for the calculation
    int r; // the result
    
    m = i / width;
    p = i - (width * p);

    if (m == 0 || p == 0 || m == width - 1 || p == height - 1) {
      results[i] = 0;
    } else {
      f = i + width;
      g = i - 1;
